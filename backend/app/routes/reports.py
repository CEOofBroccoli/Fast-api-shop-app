...
async def get_best_selling_products(
    ...
    best_selling = db.query(
        ...
        and_(
            ...
            SalesOrder.status.in_([SalesOrderStatus.CONFIRMED, SalesOrderStatus.SHIPPED, SalesOrderStatus.DELIVERED])
        )
    ).group_by(
    ...
    for product in best_selling:
        best_selling_report.append({
            ...
            "average_quantity_per_order": round(product.total_sold / product.order_count, 2) if product.order_count > 0 else 0
        })
...
async def get_supplier_ratings(
    ...
    for supplier in suppliers:
        ...
        on_time_deliveries = db.query(func.count(PurchaseOrder.id)).filter(
            and_(
                PurchaseOrder.supplier_id == supplier.id,
        ...
        completed_orders = db.query(func.count(PurchaseOrder.id)).filter(
            and_(
                PurchaseOrder.supplier_id == supplier.id,
                PurchaseOrder.actual_delivery_date.isnot(None)
            )
        ...
        total_value = db.query(func.sum(PurchaseOrder.total_cost)).filter(
            PurchaseOrder.supplier_id == supplier.id
        ).scalar() or 0
    ...
    return {
        ...
        "summary": {
            ...
            "average_rating": round(sum(s["rating"] for s in supplier_ratings) / len(supplier_ratings), 1) if supplier_ratings else 0,
            ...
            "overall_on_time_rate": round(
                ...
                max(sum(s["completed_orders"] for s in supplier_ratings), 1) * 100, 1
            )
        }
    }
