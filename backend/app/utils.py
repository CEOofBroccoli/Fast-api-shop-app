...
def validate_positive_integer(value: int, field_name: str) -> int:
    ...
    if value <= 0:
        raise ValidationError(f"{field_name} must be a positive integer", "INVALID_VALUE", {"field": field_name, "value": value})
...
def validate_pagination(page: int, limit: int) -> Tuple[int, int]:
    ...
    if limit > 100:
        raise ValidationError("Limit cannot exceed 100 items per page", "INVALID_LIMIT", {"limit": limit, "max_allowed": 100})
...
def validate_sku(sku: str) -> str:
    ...
    if len(sku) > 50:
        raise ValidationError("SKU cannot exceed 50 characters", "INVALID_SKU_LENGTH", {"length": len(sku), "max_length": 50})
    
    if not re.match(r'^[A-Z0-9_-]+$', sku.upper()):
        raise ValidationError("SKU must contain only uppercase letters, numbers, hyphens, and underscores", "INVALID_SKU_FORMAT")
