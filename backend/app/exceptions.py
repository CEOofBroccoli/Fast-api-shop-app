...
class AuthenticationError(BaseCustomException):
    ...
    def __init__(self, message: str = "Authentication failed", error_code: Optional[str] = None, details: Optional[Dict[str, Any]] = None):
        super().__init__(message, error_code, details)

class AuthorizationError(BaseCustomException):
    """Authorization related errors"""
    def __init__(self, message: str = "Access denied", error_code: Optional[str] = None, details: Optional[Dict[str, Any]] = None):
        super().__init__(message, error_code, details)
...
class DuplicateResourceError(BaseCustomException):
    ...
    def __init__(self, resource_type: str, field: str, value: Any):
        ...
        super().__init__(message, "DUPLICATE_RESOURCE", {"resource_type": resource_type, "field": field, "value": value})
...
class InventoryBusinessError(BusinessLogicError):
    ...
    def __init__(self, message: str, error_code: str = "INVENTORY_BUSINESS_ERROR", details: Optional[Dict[str, Any]] = None):
        super().__init__(message, error_code, details)
...
async def database_exception_handler(request: Request, exc: SQLAlchemyError):
    ...
    logger.error(f"Database error [{error_id}]: {sanitize_log_input(str(exc))}", extra={
        "error_id": error_id,
...
async def integrity_exception_handler(request: Request, exc: IntegrityError):
    ...
    logger.error(f"Database integrity error [{error_id}]: {sanitize_log_input(str(exc))}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "exception_type": type(exc).__name__
...
async def pydantic_validation_exception_handler(request: Request, exc: PydanticValidationError):
    ...
    logger.warning(f"Pydantic validation error [{error_id}]: {sanitize_log_input(str(exc))}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
...
async def authentication_exception_handler(request: Request, exc: AuthenticationError):
    ...
    logger.warning(f"Authentication error [{error_id}]: {sanitize_log_input(exc.message)}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
...
async def authorization_exception_handler(request: Request, exc: AuthorizationError):
    ...
    logger.warning(f"Authorization error [{error_id}]: {sanitize_log_input(exc.message)}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "error_code": sanitize_log_input(exc.error_code)
...
async def business_logic_exception_handler(request: Request, exc: BusinessLogicError):
    ...
    logger.warning(f"Business logic error [{error_id}]: {sanitize_log_input(exc.message)}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "error_code": sanitize_log_input(exc.error_code),
...
async def resource_not_found_exception_handler(request: Request, exc: ResourceNotFoundError):
    ...
    logger.info(f"Resource not found [{error_id}]: {sanitize_log_input(exc.message)}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "error_code": sanitize_log_input(exc.error_code),
...
async def duplicate_resource_exception_handler(request: Request, exc: DuplicateResourceError):
    ...
    logger.warning(f"Duplicate resource error [{error_id}]: {sanitize_log_input(exc.message)}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "error_code": sanitize_log_input(exc.error_code),
...
async def rate_limit_exception_handler(request: Request, exc: RateLimitError):
    ...
    logger.warning(f"Rate limit error [{error_id}]: {sanitize_log_input(exc.message)}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "error_code": sanitize_log_input(exc.error_code)
...
async def external_service_exception_handler(request: Request, exc: ExternalServiceError):
    ...
    logger.error(f"External service error [{error_id}]: {sanitize_log_input(exc.message)}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "error_code": sanitize_log_input(exc.error_code),
...
async def jwt_exception_handler(request: Request, exc: JWTError):
    ...
    logger.warning(f"JWT error [{error_id}]: {sanitize_log_input(str(exc))}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method)
    })
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    logger.warning(f"HTTP exception [{error_id}]: {sanitize_log_input(str(exc.detail))}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
...
async def general_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(f"Unhandled exception [{error_id}]: {sanitize_log_input(str(exc))}", extra={
        "error_id": error_id,
        "url": sanitize_log_input(str(request.url)),
        "method": sanitize_log_input(request.method),
        "exception_type": type(exc).__name__,
